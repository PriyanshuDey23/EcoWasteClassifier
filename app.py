...
def generate_carbon_footprint_info(label):
    """
    Generates carbon footprint information for a given label.

    Args:
        label (str): The label for which to generate carbon footprint information.

    Returns:
        str: The generated carbon footprint information.
    """

    # Extract the relevant part of the label
    label = label.split(' ')[1]
    print(label)
    ...
    try:
        # Call the OpenAI API
        response = openai.completions.create(**api_params)
    ...
    if st.button("Classify"):
        ...
        with col2:
            ...
            if label == "0 cardboard\n":
                st.success("The image is classified as Carboard.")                
                with col4:
